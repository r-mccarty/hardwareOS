...
- Go backend lives in the repo root with entry points in `main.go` and `cmd/`, plus shared packages in `internal/` and `pkg/`.
...
- `./dev_deploy.sh -r <IP> --skip-ui-build`: faster backend-only iteration.
...
- UI strings must be localized: add keys to `ui/localization/messages/en.json` using `snake_case`, then use `m.key_name()` in `.tsx`.
